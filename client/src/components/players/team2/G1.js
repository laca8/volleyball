...
const H1 = ({ show, handleClose, moves, num }) => {
  ...
  useEffect(() => {
    const allFields = document.querySelectorAll(".input2");
    var c = document.getElementById("myCanvas");
    const allFields2 = document.querySelectorAll(".input");
    var line = document.createElement("line");

    for (let i = 0; i < allFields.length; i++) {
      //console.log(allFields[i].value);
      for (let j = 0; j < allFields2.length; j++) {
        m?.filter((x) => x.ball == "G" && x.skill == "A" && x.num == num)?.map(
          (x) => {
  ...
  useEffect(() => {
    const allFields = document.querySelectorAll(".input22");
    var c = document.getElementById("myCanvas1");
    const allFields2 = document.querySelectorAll(".input11");
    var line = document.createElement("line");

    for (let i = 0; i < allFields.length; i++) {
      //console.log(allFields[i].value);
      for (let j = 0; j < allFields2.length; j++) {
        m?.filter((x) => x.ball == "G" && x.skill == "S" && x.num == num)?.map(
          (x) => {
            if (allFields2[j].value == x.from && allFields[i].value == x.to) {
              //var ctx = c.getContext("2d");
              //console.log(allFields2[j].offsetLeft, allFields2[j].offsetTop);
        ...
        <div style={{ display: "flex", justifyContent: "space-around" }}>
          <div style={{ width: "240px" }}>
            ...
            <div style={{ maxHeight: "150px", overflowY: "scroll" }}>
              {m
                ?.filter((x) => x.ball == "G" && x.num == num && x.skill == "A")
                ?.map((x) => (
                  <div
                    style={{
                      display: "flex",
          ...
          <div style={{ width: "240px" }}>
            ...
            <div>
              {m
                ?.filter((x) => x.ball == "G" && x.num == num && x.skill == "S")
                ?.map((x) => (
                  <div
                    style={{
                      display: "flex",
