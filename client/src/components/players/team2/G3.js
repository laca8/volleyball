...
const H9 = ({ show, handleClose, moves, num }) => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < allFields.length; i++) {
      ...
      for (let j = 0; j < allFields2.length; j++) {
        m?.filter((x) => x.ball == "G" && x.num == num)?.map((x) => {
  ...
  return (
    <Container>
      <Modal
        show={show}
        onHide={handleClose}
        style={{ width: "240px", marginLeft: "50px", fontSize: "10px" }}
      >
        ...
        <Modal.Body style={{ backgroundColor: "orange", position: "relative" }}>
